...
fore_plot_dt <- function(mod, plot = 'level') {
    ...
    if (plot == 'level') {
        dt <- data.table(quarter = c(time(y_qu), time(freq_fore$f)),
                     ...
                     forecast = c(rep(NA, (length(y_qu) -1)), y_qu[length(y_qu)], fore))
    } else {
        dt <- data.table(quarter = c(time(y_qu), time(freq_fore$f)),
                     mod = mod,
    ...
    dt_long <- melt(dt, id.vars = c('quarter', 'mod'))[quarter > max(time(y_qu)) & variable == 'forecast', forecast_pl := as.numeric(pl)][quarter > max(time(y_qu)) & variable == 'forecast', forecast_pu := as.numeric(pu)]
    return(dt_long)
}
...
gibbs_est <- function(y, a.y, b.y, a.theta, b.theta, shape.y, rate.y,
                     ...
                     dV = 1e-7, m0 = c(level0, slope0, rep(0,5)),
                     C0 = diag(x=c(rep(2,2), rep(1e7,5))),
