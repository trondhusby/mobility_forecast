...
\begin{frame}{Projecting growth components: models}
    \begin{minipage}[t]{0.48\linewidth}%
      Let's say we want to project internal migration frequency (moves per 1000 inhabitants) into the future. Three possible models:
      \small
      \begin{itemize}
        \item $\text{frequency}_{t+1} = f(\text{frequency}_{t}, \text{frequency}_{t-1}, ...,\text{error}_{t})$ \\
      \item $\text{frequency} = f(\text{housing-market}, \text{labour-market}, \text{month}, ...,\text{error})$ \\
        \item $\text{frequency}_{t+1} = f(\text{frequency}_{t}, \text{housing-market}, \text{labour-market}, \text{month}, ...,\text{error}_{t})$
        \end{itemize}
\end{minipage}%
\hfill%
\begin{minipage}[t]{0.48\linewidth}
  \vspace{-1cm}
  \centering
  \includegraphics[scale = 0.35]{\figs/{freq--freq-plot-1.pdf}}    
\end{minipage}    

\end{frame}

\begin{frame}{Projecting growth components: time-series decomposition}
    \begin{minipage}[t]{0.48\linewidth}%
      \textbf{Trend + seasonal + cycle + error}
      \small
      \begin{itemize}
      \item \textit{Trend}: a long-term increase or decrease in the data, does not have to be linear
      \item \textit{Seasonal}: time series is affected by seasonal factors such as the time of the year or the day of the week. Seasonality is always of a fixed and known frequency.
        \item \textit{Cycle}: data exhibit rises and falls that are not of a fixed frequency. Fluctuations are usually due to economic conditions
        \end{itemize}
...
\begin{minipage}[t]{0.48\linewidth}
  \vspace{0cm}
  \centering
  \includegraphics[scale = 0.4]{\figs/{freq--dlm-smoothed-1.pdf}}    
\end{minipage}    
...
\begin{frame}{Total fertility rate in 2016 and 2035}
  ...
  \begin{minipage}[t]{0.48\linewidth}%
    \begin{figure}
    \includegraphics[scale = 0.4]{\figs/{tfr_2016_P2019.png}}
...
\begin{minipage}[t]{0.48\linewidth}
  %\vspace{-1cm}
  \begin{figure}
    \includegraphics[scale = 0.4]{\figs/{tfr_2035_P2019.png}}
    %\includegraphics[scale = 0.09]{\figs/{tfr_2035_P2019.png}}
  \end{figure}
\end{minipage}
