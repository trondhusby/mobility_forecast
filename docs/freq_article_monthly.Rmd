...
This procedure is shown in Figure \@ref(fig:rolling-forecast-example) [@hyndman2018forecasting]. The movement along the x-axis illustrates how the origins (blue dots with red border) are "rolling" forward in time, where the model is estimated on the training data (blue and red dots) and evaluated on the test data (grey dots). The model perfomance is then evaluated by averaging the prediction errors over the forecast horizon (grey dots with black border). 
...
* Difference DLMs: although DLM2 is clearly better within-sample, difference in terms of forecasting performance is not particularly big.
