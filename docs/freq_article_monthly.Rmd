...
## test <- forecast(ets_fore_info, simulate = T, bootstrap = T)
## summary(ets_fore_info)
## coefficients(ets_fore_info)[1:4]

## monte carlo forecasts for three-month period in 2017
mc_out <- rbindlist(mclapply(seq(2017 + 2/12, 2017 + 11/12, by = 3/12),
                             function(x) {
                                 mc_run(orig = x, new = 3000)
                             },
                             mc.cores = n_cores
                             ))

## create readable origin name
mc_out[,
...
est_tbl <- data.table(Parameter = c("$\\sigma_{v}^{2}$", "$\\sigma_{\\mu}^{2}$",
                              ...
                              "$\\phi_{1}$", "$\\phi_{2}$",
                              "$\\phi_{7}$", "$\\phi_{12}$"),
                           DLM1 = as.character(signif(dlm1_pars, 4)),
                           DLM2 = as.character(signif(dlm2_pars, 4))
                      )
...
```{r mc-intervals, fig.cap = "Monte Carlo"}
...
ggplot(data.table(year = floor(time(window(y_m, start = 2010))),
        ...
        aes(year, V1)) +
     geom_line(data = mc_out[x %in% seq(1, 100)], aes(year, V2, group = x), col =  'grey80', alpha = 0.5) + 
     geom_line(data = mc_out[x == 1], aes(year, mean_fore, group = x)) +
     geom_line(data = mc_out[x ==1], aes(year, lo_fore, group = x), lty = 'dotted') +
    geom_line(data = mc_out[x == 1], aes(year, up_fore, group = x), lty = 'dotted') +
    geom_line(col = 'grey50') +
