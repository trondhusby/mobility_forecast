...
measure_dict <- data.table(variable = c('rmse', 'mape', 'p_within_80', 'p_within_95'),
                           measure = c('RMSE', 'MAPE', 'Coverage probability, 80 % PI', 'Coverage probability, 95% PI'))


error_dt[cov_prob, on = c('type', 'n')] %>%
  melt(, id.vars = c('type', 'n')) %>%
  .[measure_dict, on = 'variable'] %>%
  .[, `Forecast \n horizon` := factor(n)] %>%
  ggplot(., aes(value, type, group = `Forecast \n horizon`, shape = `Forecast \n horizon`)) +
  geom_point(size = 2) +
...
anno_dt <- data.table(measure = rep(c('data', 'mean', '80 %', '95 %'), each = 2),
...
ggplot(data.table(year = floor(time(window(y_m, start = 2010))),
  ...
  geom_text(data = anno_dt[anno == 'text' & origin_char == 'Mar 2017'],
            aes(x = x, y = y, label = measure, col = col)) +
