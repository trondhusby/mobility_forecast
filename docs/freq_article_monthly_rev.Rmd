...
ggplot(rbindlist(
  list(
    filt_plot_dt(freq_filt[['lt_arma2']])[, mod := 'DLM1'],
    filt_plot_dt(freq_filt[['lt_arma3']])[, mod := 'DLM2']
  ))[t > 2010], aes(t, f)) +
  geom_ribbon(aes(ymin = pl_80, ymax = pu_80, fill = mod), alpha = 0.2) +
  ...
  geom_text(aes(label = ifelse(mod == 'DLM1',
                               paste0('Coverage 80 % PI (DLM1) = ', cov_80),
                               NA)),
            x = 2012, y = 180000, size = 3.5, col = 'grey40') +
  geom_text(aes(label = ifelse(mod == 'DLM2',
                               paste0('Coverage 80 % PI (DLM2) = ', cov_80),
                               NA)),
            x = 2012, y = 175000, size = 3.5, col = 'grey40') +
...
```{r mape-table, results = 'show'}

lapply(c('mape_6', 'mape_12', 'mape_18'),
       function(x){
         dcast(rf_fit, 'mod ~ type', value.var = x)[,-1] %>%
           apply(., 2, function(y) c(mean(y), sd(y), min(y), max(y))) %>%
           cbind(data.table(.), c('mean', 'SD', 'min', 'max')) %>%
           melt(., id.vars = 'V3', variable.name = 'model') %>%
           dcast(., 'model ~ V3', mean) %>%
           .[, horizon := c(gsub('mape_', '', x), rep('', .N-1))] %>%
           setcolorder(., c(6, 1, 4, 2, 5, 3))  
       }) %>%
  rbindlist() %>%
  knitr::kable(.,
               booktabs = T, escape = F, digits = c(0, 0, 4, 4, 4, 4),
               caption = 'Mean, standard deviation, min and max of prediction errors')
