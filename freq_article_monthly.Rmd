...
##qqnorm(residuals(freq_filt[[mods[7]]])$res)
##qqline(residuals(freq_filt[[mods[7]]])$res)
...
In this paper we focus on the national-level frequency of mobility, defined as the sum of intra- and inter-municipal moves per 1000 inhabitants, representing a rough measure of the intensity of internal migration in the Netherlands. Data from the Statistics Netherlands allow us to calculate monthly time series of the frequency of mobility, from January 1995 to October 2018, meaning we have in total 286 observations at our disposal.

In this paper I firstly show the time series data and discuss its patterns. Next I present the model, discuss its elements and the estimated parameters. Finally I use the model for a series of out-of-sample forecasts along a peak of the business cycle, showing model performance for the separate forecasts. In order to assess the performance, the prediction accuracy of the DLM model is compared to that from forecasts obtained with three other models.
...
ggplot(melt(data.table(month = time(y_m),
    ...
    scale_x_continuous(breaks = 1995:2018, labels = paste('Jan ', 1995:2018))
...
Interestingly the lower-panel figure reveals that the year on year changes are negative from medio 2017. This is also reported by Statistics Netherlands [@cbs2019]: in 2018 there were 5 \% fewer moves than in 2017, with the decline concentrated primarily among people younger than 50 years old. This reflects thus an end of an increasing trend from 2014 onwards. It is precisely this apparent trend-change that we use to motivate this study. 

<!-- Another way of desentangling the cycle from a time series is the so-called Hodrick-Prescott filter, primarily used for removing cyclical components to extract a true trend. Figure \@ref(fig:freq-cycle-plot) shows the trend and the cyclical component of the frequency of mobility, extracted with the Hodrick-Prescott with a $\lambda = 125000$. In the right panel the blue line  loess smoothing to correct for seasonal variation. The figure suggests that the time series follows a cycle with peaks roughly every ten years, with the last peak around 2017, and that double dip recession between 2010 to 2014 was reflected in a decline in the frequency of mobility. The filtered trend shows that there is no clear linear trend in the data. --> 

```{r freq-cycle-plot, fig.cap = 'Trend and cycle of the frequency of mobility, extracted with the Hodrick Prescott filter'}

## ggplot(melt(data.table(month = time(y_m),
##                        trend = as.numeric(hpfilter(stl(y_m, t.window=10, s.window="periodic")$time.series[,2])$trend),
##                        cycle = as.numeric(hpfilter(stl(y_m, t.window=10, s.window="periodic")$time.series[,2])$cycle)),
##           id.vars = 'month'), aes(month, value)) +
##     geom_line() +
...
## ggplot(melt(data.table(month = time(y_m),
##                        trend = as.numeric(hpfilter(y_m, freq = 125000, type = 'lambda')$trend),
##                        cycle = as.numeric(hpfilter(y_m, freq = 125000, type = 'lambda')$cycle)),
##           id.vars = 'month'), aes(month, value)) +
##     geom_line() +
##     facet_wrap(~variable, scales = 'free') +
...
In addition to the cycle, there are strong seasonal patterns that seem to vary over time. Figure \@ref(fig:freq-month-plot) shows the distribution of the frequency of mobility per month, with the year of each observation indicated with colours (upper panel), and a the movement per year and average per month (lower panel). The figure shows that the highest frequency is, on average, in August and July while the lowest is in April. We also see there is quite some variation between the years: the differences between the months were more pronounced in the early part of the time series than in later years. The lower panel suggests a sinusoidal pattern with two peaks within one year - one peak in the summer and one at the end of the year. 

```{r freq-month-plot, fig.cap = 'Frequency per month and year'}
...
Seasonality is dealt with by including a trigonometric seasonal components to the model. One big advantage with the trigonometric specification, relative to simpler seasonal dummies, is that we allow autocorrelation to last through more lags, resulting in a smoother seasonal pattern. This way we filter out some of the noise seen in the figures in the previous section. By including non-zero variances, the seasonal pattern is allowed to change trough time. Seasonality is incorporated in the DLM model by extending the state equation with an underlying periodic process $g(t)$
...
knitr::kable(est_tbl,
             booktabs = T, escape = F, digits = c(0, 8, 8),
...
The one-step ahead predictions from the Kalman filter, or filtered values, from both of the models are displayed in Figure \@ref(fig:dlm-filtered), together with the (within-sample) Root Mean Square Error (RMSE) of the predictions. Note that we have taken the anti-log of the filtered values, since this is our primary interest here. As is seen in the figure, in the first time periods the volatility of the predicted time-series exceeeds that of the observed values, and the filtered values drift quite far off the observed values. This reflects the \textit{adaptive} nature of the Kalman algorithm, and when calculating the RMSE the two first years are discarded. It is also clear from the figure that DLM2, with the two higher-order autoregressive terms, performs better than the DLM1. 

```{r dlm-filtered, fig.cap = 'One-step-ahead predictions (blue line), observed data (red) and RMSE (calculated from January 1998)'}
...
p1 <- ggplot(data.table(f = exp(freq_filt[[mods[7]]]$f),
                  #pl = exp(sqrt(unlist(freq_fore$Q))),
           t = as.numeric(time(y_m)),
           y = y_m,
           m = rmse(window(y_m, start = c(1998, 1)),
                    window(exp(freq_filt[[mods[7]]]$f), start = c(1998, 1))
                    )
           ), aes(t, y)) +
    geom_line(col = brewer.pal(3, 'Set1')[2]) +
    geom_line(aes(y = f), col = 'red') +
    geom_text(label = 'RMSE', x = 2014, y = 10.5, col = 'grey40') +
    geom_text(aes(label = round(m, 4)), x = 2014, y = 9.5, col = 'grey40') +
    ylab('Frequency of mobility') +
    theme_bw() +
    ggtitle('DLM1')

p2 <- ggplot(data.table(f = exp(freq_filt[[mods[8]]]$f),
                  #pl = exp(sqrt(unlist(freq_fore$Q))),
           t = as.numeric(time(y_m)),
           y = y_m,
           m = rmse(window(y_m, start = c(1998, 1)),
                    window(exp(freq_filt[[mods[8]]]$f), start = c(1998, 1))
                    )
           ), aes(t, y)) +
    geom_line(col = brewer.pal(3, 'Set1')[2]) +
    geom_line(aes(y = f), col = 'red') +
...
ggplot(plot_dt, aes(mod, train)) +
    ...
    geom_point(aes(y = test), col = 'grey', size = 4) +
    geom_point(aes(y = origin), col = pal[1], size = 4) +
    theme_bw() +
    theme(
...
knitr::kable(roll_pars,
             booktabs = T, escape = F, digits = c(0, 8, 8, 8, 8),
...
rmse_mean <- rbindlist(list(err_dt('rmse_6')[19:20],
               err_dt('rmse_12')[19:20],
               err_dt('rmse_18')[19:20])
          )
...
out <- rbindlist(list(err_dt('rmse_6')[-c(19:20)],
                 err_dt('rmse_12')[-c(19:20)],
                 err_dt('rmse_18')[-c(19:20)]
                 ))[,
                   N := rep(paste0('N=', c(6, 12, 18)), each = 18)
                   ][,
...
ggplot(melt(out, id.vars = c('Origin', 'N')),
    ...
    scale_x_discrete(breaks = unique(out$Origin)[seq(1, 18, by = 3)])
...
In this study we have developed a DLM...
